...
export const behaviors:Record<string,Behavior>={
  ...
  regeneration:{
    tick({ball}){ball.mintRest=(ball.mintRest??0)+1;if(ball.mintRest>120)ball.hp=Math.min(100,ball.hp+.07);},
  ...
  compoundPower:{
    ...
    dealHit({ball,event,showImpact}){if(event.jackpot&&event.damage>0){ball.goldStacks=0;showImpact('JACKPOT!',ball);}else if(!event.jackpot)ball.goldStacks=Math.min(5,(ball.goldStacks??0)+1);},
  ...
  siphon:{
    dealHit({ball,event,showImpact}){const ratio=ball.hp<50?.36:.18;ball.hp=Math.min(100,ball.hp+event.damage*ratio);if(ratio>.2)showImpact('FEAST!',ball);},
  ...
  thorns:{
    takeHit({ball,rival,event,showImpact}){if(event.damage<=0)return;rival.hp-=event.damage*.3;ball.sporeMeter=(ball.sporeMeter??0)+event.damage;if(ball.sporeMeter>=18){ball.sporeMeter=0;rival.stunned+=30;showImpact('ROOTED!',rival);}},
