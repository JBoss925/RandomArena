...
export const behaviors:Record<string,Behavior>={
  ...
  armor:{
    ...
    takeHit({ball,event,showImpact,emitParticles,playSound}){if(event.damage<=0||(ball.armorPlates??0)<=0)return;pulse(ball,'steel',12);ball.armorPlates=Math.max(0,(ball.armorPlates??0)-(event.armorDamage??1));emitParticles(ball,{count:6,color:'#d7dde2',speed:180,gravity:260,kind:'shield',size:6});playSound('armorBlock');if(ball.armorPlates===0){ball.armorRepair=180;pulse(ball,'armorBreak',28);showImpact('ARMOR BREAK!',ball);emitParticles(ball,{count:18,color:'#f3efdf',speed:390,gravity:420,kind:'shield',size:9});playSound('armorBreak');}},
  ...
  regeneration:{
    tick({ball,emitParticles,playSound}){ball.mintRest=(ball.mintRest??0)+1;if(ball.mintRest===121){pulse(ball,'healing',45);emitParticles(ball,{count:12,color:'#abf1dd',speed:150,gravity:-90,kind:'heal',size:8});playSound('heal');}if(ball.mintRest>120){applyHealing(ball,.07);if(ball.mintRest%30===0)emitParticles(ball,{count:2,color:'#abf1dd',speed:70,gravity:-60,kind:'heal',size:7});}},
  ...
  siphon:{
    dealHit({ball,rival,event,showImpact,emitParticles,playSound}){const ratio=(ball.hp<50?.36:.18)*(event.healingScale??1);applyHealing(ball,event.damage*ratio);pulse(ball,'siphon',16);emitParticles({x:(ball.x+rival.x)/2,y:(ball.y+rival.y)/2},{count:5,color:'#a4a4a4',speed:120,gravity:0,kind:'void',size:7});playSound('siphon',{volume:ratio>.2?1:.55});if(ratio>.2)showImpact('FEAST!',ball);},
  ...
  bubbleShield:{
    ...
    modifyIncoming({ball,event}){if(ball.bubbleShield){event.damage*=event.shieldPenetration??0;event.bubblePop=true;ball.bubbleShield=false;ball.bubbleRecharge=240;}},
  ...
  coldSnap:{
    modifyOutgoing({sim,rival,event}){if((rival.frostFrozenUntil??0)>sim.ticks){event.damage+=16;event.iceShatter=true;event.damageType='ice';}},
    dealHit({sim,rival,event,showImpact,emitParticles,playSound}){if(event.iceShatter){rival.frostFrozenUntil=0;rival.stunned=0;pulse(rival,'iceShatter',26);showImpact('ICE SHATTER!',rival);emitParticles(rival,{count:30,color:'#d8f7ff',speed:470,gravity:300,kind:'ice',size:10});playSound('iceShatter');}else if(event.force>8){rival.stunned+=90;rival.frostFrozenUntil=sim.ticks+rival.stunned;pulse(rival,'freezeIn',24);showImpact('FROZEN!',rival);emitParticles(rival,{count:14,color:'#d8f7ff',speed:180,gravity:-30,kind:'snowflake',size:7});playSound('freeze');}},
  },
  afterburn:{
    modifyOutgoing({rival,event}){if((rival.burnStacks??0)>=2){event.damage+=10;event.ignite=true;event.damageType='burn';}},
  ...
  venom:{
    dealHit({rival,event,showImpact,emitParticles,playSound}){
      if((event.damage<=0&&!event.bubblePop)||event.echo)return;
      rival.poisonStacks=(rival.poisonStacks??0)+1;
      rival.poisonTick=0;
      pulse(rival,'poisoned',36);
      showImpact(`VENOM ×${rival.poisonStacks}`,rival);
      emitParticles(rival,{count:8+Math.min(8,rival.poisonStacks),color:'#245c2a',speed:180,gravity:80,kind:'poison',size:7});
      playSound('poison',{volume:.72,rate:.88+Math.min(.3,rival.poisonStacks*.025)});
    },
    draw({ball,ctx,sim}){
      ctx.save();ctx.strokeStyle='#d7ff63';ctx.lineWidth=3;ctx.globalAlpha=.45+.25*Math.sin(sim.ticks*.12);
      ctx.setLineDash([3,8]);ctx.beginPath();ctx.arc(ball.x,ball.y,ball.radius+9,0,Math.PI*2);ctx.stroke();ctx.restore();
    },
  },
  echo:{dealHit({ball,sim,rival,event,showImpact,emitParticles,playSound}){if(event.damage<=0||event.echo)return;sim.echoes.push({attacker:ball,victim:rival,frames:14,damage:event.damage*.35},{attacker:ball,victim:rival,frames:32,damage:event.damage*.3});pulse(rival,'echo',34);showImpact('REVERB!',rival);emitParticles(rival,{count:10,color:'#9ce3df',speed:190,gravity:0,kind:'echo',size:8});playSound('echo');}},
  thirdHitBlock:{
    modifyIncoming({ball,event}){if(ball.incoming%3===0&&!event.unblockable){const penetration=event.parryPenetration??(event.ability?.5:0);event.blockedDamage=event.damage*(1-penetration);event.damage*=penetration;event.rookBlock=true;}},
    takeHit({ball,rival,event,showImpact,emitParticles,playSound}){if(!event.rookBlock)return;applyDamage(rival,(event.blockedDamage??0)*.3,'physical');rival.vx*=-1.08;rival.vy*=-1.08;rival.stunned+=10;pulse(ball,'steel',22);showImpact('PARRY!',ball);emitParticles(ball,{count:16,color:'#f3efdf',speed:350,gravity:300,kind:'shield',size:8});playSound('armorBlock',{volume:1.15,rate:.82});},
  ...
  continuousAcceleration:{
    ...
    dealHit({ball,emitParticles,playSound}){ball.vx*=.86;ball.vy*=.86;pulse(ball,'cometImpact',16);emitParticles(ball,{count:10,color:'#ffc2d4',speed:300,gravity:0,kind:'comet',size:7});playSound('whoosh');},
    drawBack({ball,ctx}){
  ...
  woundedPower:{
    tick({ball,showImpact,emitParticles,playSound}){ball.mass=ball.f.mass*(1+(100-Math.max(0,ball.hp))/125);if(ball.hp<50){pulse(ball,'heavy',2);if(!ball.lastStandAwakened){ball.lastStandAwakened=true;pulse(ball,'lastStand',48);showImpact('LAST STAND!',ball);emitParticles(ball,{count:24,color:'#b9c8d2',speed:280,gravity:360,kind:'metal',size:9});playSound('lastStand');}}},
    ...
    draw({ball,ctx,sim}){const p=Math.max(0,(100-ball.hp)/100);if(!p)return;ctx.save();ctx.strokeStyle='#b9c8d2';ctx.lineWidth=3+p*5;ctx.beginPath();ctx.arc(ball.x,ball.y,ball.radius+9,0,Math.PI*2);ctx.stroke();if((ball.visualStates.lastStand??0)>0){ctx.globalAlpha=.65;for(let i=0;i<8;i++){const a=i*Math.PI/4+sim.ticks*.025,r=ball.radius+18+Math.sin(sim.ticks*.2+i)*5;ctx.beginPath();ctx.moveTo(ball.x+Math.cos(a)*r,ball.y+Math.sin(a)*r);ctx.lineTo(ball.x+Math.cos(a)*(r+14),ball.y+Math.sin(a)*(r+14));ctx.stroke();}}ctx.restore();},
  },
  orbitalSatellites:{
    tick({ball,sim,rival,showImpact,random,audioTone,audioHit,emitParticles,playSound}){ball.orbitCooldown=Math.max(0,(ball.orbitCooldown??0)-1);const radius=ball.radius+31;for(let i=0;i<2;i++){const a=sim.ticks*.065+i*Math.PI,sx=ball.x+Math.cos(a)*radius,sy=ball.y+Math.sin(a)*radius,dx=rival.x-sx,dy=rival.y-sy,d=Math.hypot(dx,dy);if(d<rival.radius+12&&!ball.orbitCooldown){rival.incoming++;const event={damage:5,force:5,ability:true,damageType:'electric' as const};const context={sim,rival:ball,event,random,showImpact,audioTone,audioHit,emitParticles,playSound};runBehaviorHook(rival,'modifyIncoming',context);applyDamage(rival,event.damage,event.damageType);runBehaviorHook(rival,'takeHit',context);rival.vx+=dx/(d||1)*65;rival.vy+=dy/(d||1)*65;rival.flash=6;ball.orbitCooldown=24;pulse(rival,'satellite',12);showImpact('SATELLITE!',{x:sx,y:sy});emitParticles({x:sx,y:sy},{count:10,color:i?'#c9bdff':'#e6ff34',speed:260,gravity:0,kind:'moon',size:7});playSound('electric',{volume:.55,rate:1.4});break;}}},
  ...
  slugger:{dealHit({rival,event,showImpact,emitParticles,playSound}){if(!event.weapon)return;rival.wallCrash={frames:90,damage:12};pulse(rival,'launched',20);showImpact('WALL SLAM ARMED!',rival);emitParticles(rival,{count:14,color:'#f1c590',speed:360,gravity:250,kind:'splinter',size:8});playSound('bat');}},
  joust:{
    ...
    modifyOutgoing({ball,rival,event}){if((ball.joustFrames??0)>0&&event.weapon){event.damage+=ball.joustDamage??12;if((rival.mass??rival.f.mass)>=1.25)event.damage*=1.2;event.unblockable=true;}},
  ...
  growth:{
    wallHit({ball,showImpact,emitParticles,playSound}){if(!growBall(ball,1.35))return;pulse(ball,'growth',14);emitParticles(ball,{count:5,color:'#b8ff82',speed:90,gravity:-30,kind:'growth',size:6});playSound('grow',{volume:.42,rate:1.25});if(Math.round(ball.radius)%10===0)showImpact('BIGGER!',ball);},
    dealHit({ball,event,showImpact,emitParticles,playSound}){if(event.weapon||event.projectile||event.damage<=0||!growBall(ball,2.6))return;pulse(ball,'growth',22);showImpact('GROW!',ball);emitParticles(ball,{count:12,color:'#b8ff82',speed:150,gravity:-50,kind:'growth',size:8});playSound('grow');},
  ...
  wildFlail:{
    tick({ball,rival,sim,event,random,showImpact,emitParticles,playSound}){
      ...
      runBehaviorHook(ball,'modifyOutgoing',context);runBehaviorHook(rival,'modifyIncoming',{...context,rival:ball});applyDamage(rival,hit.damage,'physical');
  ...
  polarityDrive:{
    ...
    dealHit({ball,rival,event,showImpact,emitParticles,playSound}){
      ...
      pulse(ball,ball.polarity===1?'magnetPull':'magnetPush',24);emitParticles({x:(ball.x+rival.x)/2,y:(ball.y+rival.y)/2},{count:18,color:ball.polarity===1?'#67e8ff':'#ff6b8a',speed:260,gravity:0,kind:'magnet',size:8});
    },
  ...
  droneCarrier:{
    ...
    modifyOutgoing({event}){if(event.projectile&&event.ability)event.shieldPenetration=1.8;},
    tick({ball,rival,sim,random,showImpact,emitParticles,playSound}){
      ...
      pulse(ball,'droneLaunch',18);showImpact('SCRAMBLE!',{x,y});emitParticles({x,y},{count:12,color:ball.f.accent,speed:210,gravity:0,kind:'rocket',size:7});playSound('droneLaunch');
    },
