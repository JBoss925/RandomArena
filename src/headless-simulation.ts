...
function ctx(sim:Simulation,rival:Ball,event:CombatEvent){return{sim,rival,event,random:sim.rng,showImpact:(label:string)=>{sim.events[label]=(sim.events[label]??0)+1;},audioTone:noop,audioHit:noop};}
