...
function playSynth(kind:SynthSound,volume:number,random:RandomSource):void{
  ...
  if(kind==='electricArc'||kind==='echoPulse'||kind==='growthBloom'||kind==='magnet'||kind==='voidPulse'){
    const duration=kind==='echoPulse'?.32:.18,master=context.createGain();master.gain.setValueAtTime(Math.min(.14,volume),now);master.gain.exponentialRampToValueAtTime(.0001,now+duration);master.connect(context.destination);
    const pitches=kind==='echoPulse'?[520,390,290]:kind==='growthBloom'?[260,390,520]:kind==='magnet'?[145,154]:kind==='voidPulse'?[210,72]:[980,420,1320];
    for(const [index,pitch] of pitches.entries()){const oscillator=context.createOscillator(),gain=context.createGain(),start=now+(kind==='echoPulse'||kind==='growthBloom'?index*.045:0);oscillator.type=kind==='electricArc'?'square':kind==='voidPulse'?'sine':'triangle';oscillator.frequency.setValueAtTime(pitch,start);if(kind==='electricArc')oscillator.frequency.exponentialRampToValueAtTime(Math.max(90,pitch*.34),start+duration*.8);if(kind==='voidPulse')oscillator.frequency.exponentialRampToValueAtTime(Math.max(38,pitch*.45),start+duration);gain.gain.setValueAtTime(.5/(index+1),start);gain.gain.exponentialRampToValueAtTime(.0001,start+duration);oscillator.connect(gain).connect(master);oscillator.start(start);oscillator.stop(start+duration);}
    return;
  }
  if(kind==='slotJackpot'){
    const duration=.72,master=context.createGain();master.gain.setValueAtTime(Math.min(.15,volume),now);master.gain.setValueAtTime(Math.min(.15,volume),now+.54);master.gain.exponentialRampToValueAtTime(.0001,now+duration);master.connect(context.destination);
    // Three quick reel stops lead into a bright major-chord payout.
    for(let stop=0;stop<3;stop++){const start=now+stop*.075,oscillator=context.createOscillator(),gain=context.createGain();oscillator.type='square';oscillator.frequency.setValueAtTime(1180-stop*95,start);oscillator.frequency.exponentialRampToValueAtTime(720-stop*55,start+.035);gain.gain.setValueAtTime(.19,start);gain.gain.exponentialRampToValueAtTime(.0001,start+.055);oscillator.connect(gain).connect(master);oscillator.start(start);oscillator.stop(start+.06);}
    for(const [index,pitch] of [659.25,830.61,987.77,1318.51].entries()){const start=now+.22+index*.065,oscillator=context.createOscillator(),gain=context.createGain();oscillator.type=index===3?'square':'triangle';oscillator.frequency.setValueAtTime(pitch,start);gain.gain.setValueAtTime(.34/(1+index*.18),start);gain.gain.exponentialRampToValueAtTime(.0001,start+.25);oscillator.connect(gain).connect(master);oscillator.start(start);oscillator.stop(start+.26);}
    const thump=context.createOscillator(),thumpGain=context.createGain(),payoff=now+.47;thump.type='sine';thump.frequency.setValueAtTime(145,payoff);thump.frequency.exponentialRampToValueAtTime(58,payoff+.18);thumpGain.gain.setValueAtTime(.58,payoff);thumpGain.gain.exponentialRampToValueAtTime(.0001,payoff+.2);thump.connect(thumpGain).connect(master);thump.start(payoff);thump.stop(payoff+.21);
    return;
  }
  if(kind==='ascend'){
